...
from zoo.ocrl.env.ocrl_wrappers import wrap_lightzero

@ENV_REGISTRY.register('ocrl_lightzero')
class OCRLEnvLightZero(BaseEnv):
    """
    Overview:
    ...
    config = dict(
        ...
        # Only effective when env_manager.type is 'basgit reset --hard HEADe'.
        replay_path=None,
        ...
        stop_value=int(1e6),
        ocr_config_path='LightZero/zoo/ocr/slate/config/slate_ocrl.yaml',
    ...
    def observe(self) -> dict:
        ...
        if self.cfg.from_pixels:
            if not self.channel_last and not self.cfg.oc_model:
    ...
    def __repr__(self) -> str:
        return "LightZero OCRL Env({})".format(self.cfg.env_id)
