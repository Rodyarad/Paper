...
class ObjectMaskWrapper(gym.Wrapper):
    ...
    def _normalize(image, amplitude):
        max_value = image.max()
        if max_value == 0:
            # The object is not visible due to occlusion
            normalized_image = image
        else:
            normalized_image = image / max_value * amplitude
        ...
        self.observation_space = gym.spaces.Dict(
            {
                "robot_state": gym.spaces.Box(
                    low=-1, high=1, shape=(28,), dtype=float
                ),
                "object_states": gym.spaces.Box(
                    low=-1, high=1, shape=(40,), dtype=float
                ),
                "gt": gym.spaces.Box(low=-1, high=1, shape=(5, 40), dtype=float),
            }
        )
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "gt": gym.spaces.Box(low=-1, high=1, shape=(5, 28), dtype=float),
            }
        )
