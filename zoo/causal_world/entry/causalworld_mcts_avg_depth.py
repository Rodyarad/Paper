...
from lzero.mcts.tree_search import mcts_ctree_sampled as mcts_ctree_sampled_module
from lzero.mcts.utils import prepare_observation
from zoo.causal_world.entry.causalworld_eval import build_config
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Estimate average MCTS max-depth on sampled CausalWorld states."
    )
    parser.add_argument(
        "--model-path",
        type=str,
        default='oc_agents_weights/oz_stica_cw_slate_seed7.pth.tar',
        help="Path to trained SampledUniZero checkpoint (*.pth.tar).",
    )
...
def _build_env_and_policy(args: argparse.Namespace):
    ...
    # One synchronous env for deterministic state sampling.
...
def _to_jsonable_action(action) -> List[float]:
...
def main() -> None:
    ...
    if not getattr(cfg.policy, "mcts_ctree", True):
        raise RuntimeError("This script currently supports only ctree mode (policy.mcts_ctree=True).")

    tracker = SearchDepthTracker()
    original_batch_traverse = mcts_ctree_sampled_module.tree_sampled_muzero.batch_traverse
    ...
    mcts_ctree_sampled_module.tree_sampled_muzero.batch_traverse = wrapped_batch_traverse

    sampled_depths: List[int] = []
    sampled_timesteps: List[int] = []
    sampled_actions: List[List[float]] = []
    ...
    try:
        ...
        while len(sampled_depths) < args.num_states:
            ...
            policy_action = np.asarray(out0["action"], dtype=np.float32).reshape(-1)
            action = policy_action if args.sampling_policy == "policy" else np.asarray(
                env.action_space.sample(), dtype=np.float32
            )
            sampled_actions.append(_to_jsonable_action(action))
        ...
        mcts_ctree_sampled_module.tree_sampled_muzero.batch_traverse = original_batch_traverse
        env.close()
