...
from zoo.ocr.tools import Tensor, img_to_slot, calculate_ari, for_viz, visualize
from zoo.ocr.slate.common.utils import (
    cosine_anneal,
    gumbel_softmax,
    PositionalEmbedding,
)
from zoo.ocr.slate.common.networks import linear
from zoo.ocr.slate.common.models import SlotAttnCNNEncoder, dVAE
from zoo.ocr.slate.common.slot_attn import SlotAttentionEncoder
from zoo.ocr.slate.common.transformer import LearnedPositionalEncoding, TransformerDecoder

from zoo.ocr.slate.common.models import BroadCastDecoder
...
class SLATE_Module(nn.Module):
    ...
    def _get_slots(self, obs, prev_slots=None, with_attns=False, z_hard=None, with_ce=False):
        res = []
        emb = self._enc_pos(self._enc(obs))
        emb = emb.permute(0, 2, 3, 1).flatten(start_dim=1, end_dim=2)
        # apply slot attention
        slots, attns = self._slotattn(emb, prev_slots)
        res.append(slots)
        if with_attns:
            res.append(attns)
        if with_ce:
    ...
    def forward(self, obs: Tensor, with_attns, with_masks, prev_slots=None) -> Tensor:
        ...
        if self._use_cnn_feat:
            ...
            if with_attns or with_masks:
                ...
                return self._get_slots(obs, prev_slots=prev_slots)
    ...
    def get_loss(self, obs: Tensor, masks, with_rep=False, with_mse=False, prev_slots=None) -> dict:
        ...
        # get slots
        slots, attns, cross_entropy = self._get_slots(obs, z_hard=z_hard, with_attns=True, with_ce=True, prev_slots=prev_slots)
        attns = attns.transpose(-1, -2).reshape(
            obs.shape[0], slots.shape[1], 1, obs.shape[2], obs.shape[3]
        )
